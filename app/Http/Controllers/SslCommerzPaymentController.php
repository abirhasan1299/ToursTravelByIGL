...
class SslCommerzPaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $update_product = DB::table('transactions')
            ->where('t_transaction_id', $post_data['tran_id'])
            ->updateOrInsert([
                't_user_id' => Auth::user()->id,
                't_transaction_id' => $post_data['cus_email'],
                't_amount' => $post_data['total_amount'],
                't_status' => 'pending',
                't_transaction_id' => $post_data['tran_id'],
                't_currency' => $post_data['currency'],
                't_reference' => "Buying Credit",
                'created_at' => now(),
                'updated_at' => now()
            ]);
    ...
    public function BuyPlan(Request $request)
    {
        ...
        $post_data['total_amount'] = $package->p_price;
        $post_data['currency'] = config('app.currency');
        $post_data['tran_id'] = uniqid();
        ...
        $update_product = DB::table('transactions')
            ...
            ->updateOrInsert([
                ...
                't_transaction_id' => $post_data['tran_id'],
                't_currency' => $post_data['currency'],
                't_reference' => "Plan booked for ".$package->p_name,
                'created_at' => now(),
                'updated_at' => now()
            ]);
            ...
            if ($validation) {
                ...
                if ($transaction) {
                    ...
                    // update status
                     ...
                     //check if credited or Plan package buy
                     if($request->input('value_a')=='package')
                     {
